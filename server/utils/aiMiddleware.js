...
const aiMiddleware = {
    ...
    buildSystemPrompt({ model = '', thinking = true, region = '云南省', year = 2023 } = {}) {
        ...
        const skillsGuidance = '\n\n【专家技能库指引】\n你拥有一个 `knowledge_base_lookup` 技能库（这是“技能手册”，不是知识图谱，也不是政策文献检索）。当你对以下内容不确定时，**必须先查阅技能库**：\n- 生态监测指标的具体含义、公式或阈值（查询 `monitoring_indices`）\n- 如何解读重心迁移轨迹、标准差椭圆的空间意义（查询 `spatial_reasoning`）\n- 涉及政策因果、业务口径、风险等级映射或“为什么发生”的解释逻辑不确定时（查询 `policy_expert`）\n查阅后再进行分析，以确保结论的绝对专业性。\n\n【知识图谱与政策文献能力说明】\n- `knowledge_graph_query`：系统内置“知识图谱查询”（语义层），用于查节点与关系（例如政策节点、概念关系、行政层级）。当用户问“图谱里有没有某某政策/某概念和某概念如何关联/某节点有哪些关系”时，优先调用它。\n- `policy_reference_lookup`：系统内置“政策/规划文献索引检索”，用于检索可引用条目并返回来源链接（sources）。当用户问“某年重大政策/三条控制线/用途管制/耕地保护/规划批复”等解释时，先调用它再作解释，正文必须列出来源链接。';
