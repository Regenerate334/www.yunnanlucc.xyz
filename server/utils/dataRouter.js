/**
 * AI 数据源聚合路由器 (AI Data Aggregation Router)
 * 职责：负责根据 AI 解析出的空间维度、时间段及指标意图，调用对应的数据库服务提取精确数据。
 *
 * 修改提示：
 * 1. 它是工具调用的实际执行层，新增任何数据维度的查询逻辑（如气象、水文）都在此注册。
 * 2. 数据查询结果会被格式化为自然语言片段，用于注入给下一个大模型节点生成最终结论。
 * 3. 对于极其耗时的空间统计，务必加入超时熔断机制。
 */
/**
