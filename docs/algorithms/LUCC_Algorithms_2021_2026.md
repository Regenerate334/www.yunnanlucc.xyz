...
1. 核心问题：耕地、城镇建设、生态三者存在强耦合，单一综合分易出现“风险抵消”。  
...
- 可证据事实：InVEST Habitat Quality 是基于威胁源、敏感性、衰减与可达性的空间模型，不是简单面积加权。[2]  
- 本系统做法：使用面积加权得到“生境支持代理指标（HQ proxy）”。  
- 规范写法：不得宣称“严格等同 InVEST-HQ”，只能写“受其生态质量思想启发的代理指标”。

### 1.2 熵权法

- 可证据事实：熵权法的标准推导（$p_{ij}, e_j, d_j, w_j$）有稳定方法学来源。[7][8]  
- 本系统做法：用于四个核心指标（HQ/CMP/ERes/PLEC）的客观赋权。

### 1.3 耦合协调度（CCD）

- 可证据事实：$D=\sqrt{C\cdot T}$ 形式在区域发展、生态-经济协调研究中广泛使用。[15]  
...
1. 生境支持代理（HQ proxy，权重映射参考 HJ 192-2015 的生境质量分权重）[1]
...
> 系数口径参考“地类碳排放/碳吸收系数法”，并将建设用地占比作为压力项进行工程化修正。[3][4]
...
> 注：本系统采用“系数法代理”表达韧性，并通过区域化校准给定地类韧性系数（非统一国家标准真值）。可参考基于生态系统健康/韧性建模的相关研究作为写作背景。[5]
...
> 注：该指标为“压力比”形式的简化代理表达，用于表征生产/生活空间对生态空间的挤压程度，不等同于空间冲突指数（SCI）等景观格局原式。[6]
...
> 熵权法的“信息熵”思想源于 Shannon 熵；在多指标客观赋权中的工程化应用可参考熵权-决策方法文献。[7][8][9]
...
> 耦合协调度（CCD）属于经典的耦合-协调评判框架，本文仅将其用于 PLE 子系统的风险代理表达。[15]
...
1. 单一地类动态度（单类变化速率；常见定义可追溯至王秀兰、包玉海（1999），并在 LUCC 变化监测中被广泛采用，如 Huang et al., 2018）。[11][14]

$$
K_i=\frac{A_{i,t_2}-A_{i,t_1}}{A_{i,t_1}}\cdot\frac{1}{T}\cdot100\%
$$

2. 综合动态度（区域总体变化剧烈程度；常见定义可追溯至王秀兰、包玉海（1999），并在 LUCC 变化监测中被广泛采用，如 Huang et al., 2018）。[11][14]
...
在此基础上，文献里常用的分解口径包括“保持/转入/转出/净变化/交换变化（swap）”等概念。[12]
...
若你希望把“变化强度”做成论文主线，Intensity Analysis 是一个更系统的框架：以区间、类别、转移三个层级统一度量变化强度与稳定性；应用实例可参考 Huang et al., 2018。[13][14]  
...
> 情景划分与政策约束的制度背景，可参考国土空间规划体系与用途管制相关的顶层文件。[10]
...
15. 廖重斌. 环境与经济协调发展的定量评判及其分类体系——以珠江三角洲城市群为例[J]. *热带地理*, 1999, 19(2): 171-177.  
    链接: https://www.rddl.com.cn/CN/Y1999/V19/I2/171  
    说明: 常被用作“耦合协调度（CCD）模型”分级与判别框架的早期来源之一。
