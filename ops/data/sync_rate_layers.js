/**
 * 空间图层比例数据同步脚本 (Spatial Rate Layer Synchronization)
 * 职责：重新构建年度空间图层（县级/格网），预计算土地垦殖率与转换率，加速前端加载。
 * 
 * 修改提示：
 * 1. 本脚本会执行 DROP TABLE，操作前请确认 PostgreSQL 备份状态。
 * 2. 空间单元映射关系在 UNIT_TABLE_MAP 中定义，新增级别需在此注册。
 * 3. 运行完成后需在 GeoServer 中手动或通过脚本刷新图层缓存。
 */
