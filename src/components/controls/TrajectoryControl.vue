...
const handleQuery = () => {
  ...
  emit('stats-query', {
    ...
    showTrajectory:    true,
    showSDE:           false,
    legendTitle: `${yearStart.value}-${yearEnd.value}年 ${directionLabel} 重心轨迹`
  });
};
...
const handleClickOutside = (event) => {
  ...
  const isTrigger = path.some(
    el => el.classList
      && (el.classList.contains('spatial-stats-entry-btn') || el.classList.contains('trajectory-btn-trigger'))
  );
...
<style scoped>
/* 复用样式，但由于是单文件组件，我们需要确保样式能正确应用。
   在实际开发中，可以将公共样式抽离到单独的 CSS 文件中。
   这里为了保持一致性，我将直接复制核心样式。
*/
