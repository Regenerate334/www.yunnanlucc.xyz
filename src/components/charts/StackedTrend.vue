...
function render(){
  ...
  const series = classNames.map(name=>({
    name,
    type:'line', stack: usePercent.value ? 'pct' : 'total', showSymbol:false, areaStyle:{ opacity:0.25 },
  ...
  let yAxis = { type:'value', name: usePercent.value ? '%' : 'km²' };
  let option = {
...
.empty{ height:300px; display:flex; align-items:center; justify-content:center; color:#94a3b8; }
</style>
