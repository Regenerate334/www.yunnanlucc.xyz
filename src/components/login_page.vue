<template>
  <div class="relative flex h-screen w-screen items-center justify-center overflow-hidden">
    ...
    <div class="pointer-events-none absolute inset-0">
      ...
      <div class="absolute inset-0" style="background-image: linear-gradient(rgba(255,255,255,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.06) 1px, transparent 1px); background-size: 40px 40px;"></div>
    </div>
    ...
    <div class="relative z-10 flex flex-col items-center text-center text-white px-4">
      ...
      <div class="mb-10 flex flex-wrap items-center justify-center gap-4">
        ...
        >
          <span class="absolute -inset-full top-0 block -translate-x-full transform bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-500 ease-in-out group-hover:translate-x-full"></span>
          进入平台
        </button>
        <a
          href="https://cesium.com/learn/cesiumjs-learn/" target="_blank" rel="noreferrer"
      ...
      <div class="mb-10 grid w-full max-w-4xl grid-cols-1 gap-4 px-2 md:grid-cols-3">
        <div class="rounded-xl border border-white/10 bg-white/5 p-4 text-center shadow-sm backdrop-blur-md flex flex-col items-center">
        ...
        <div class="rounded-xl border border-white/10 bg-white/5 p-4 text-left shadow-sm backdrop-blur-md flex flex-col items-center">
...
import { useRouter } from 'vue-router'; // 页面路由
import bgUrl from '../assets/login_page_background.png'; // 背景图
...
// 背景缩放：保持与页面缩放一致（优先监听 visualViewport.scale）
